...
#define  VER         1.6                //  ВЕРСИЯ ПО НЕ ТРОГАЙ/ ЕСЛИ ЧТО-ТО ИЗМЕНЯЕТЕ В КОДЕ, СООБЩИТЕ МНЕ!!!
...
void para (){
  ...
  if(time.Hours == 9){
      ...
      if(time.minutes == 50){
      disp.set(LED_0F[23], 3);
      disp.dispFloat(-1.5, 1); //индикация с точкой
      }
      }
      if(time.Hours == 10){
      if(time.minutes == 45){
    ...
    if(time.Hours == 9){
      ...
      disp.set(LED_0F[28], 0);
      delay(50);  
      disp.dispFloat(-5.0, 1); //индикация с точкой
      }
      }
      delay(1000);
    }
